...
# Grant secret access to service account (for AISR credentials)
resource "google_secret_manager_secret_iam_member" "function_secret_access" {
  for_each = toset(["aisr-username", "aisr-password"])
  
  secret_id = each.value
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.function_sa.email}"
  
  depends_on = [google_secret_manager_secret.aisr_secrets]
}

# Secrets for AISR credentials (create empty - user fills in via console)
resource "google_secret_manager_secret" "aisr_secrets" {
  for_each = toset(["aisr-username", "aisr-password"])
...
resource "google_cloudfunctions2_function" "upload_function" {
  ...
  service_config {
    ...
    environment_variables = {
      ...
      AISR_USERNAME_SECRET  = "aisr-username"
      AISR_PASSWORD_SECRET  = "aisr-password"
    }
  }
