...
def generate_fake_data(num_rows: int, seed: int = 42) -> pd.DataFrame:
    ...
    data = {
        ...
        'birth_date': [fake.date_of_birth(minimum_age=5, maximum_age=18).strftime('%Y-%m-%d') for _ in range(num_rows)],
        'sex_code': [random.choice(['M', 'F']) for _ in range(num_rows)],
        'race_code': [random.choice(['A', 'B', 'C', 'D', 'E', 'F']) for _ in range(num_rows)],  # Made-up race codes
        'ethnicity_code': [random.choice(['NH', 'U']) for _ in range(num_rows)],
        'immunization_id': [random.randint(10000000, 99999999) for _ in range(num_rows)],
        ...
        'vaccination_date': [fake.date_this_decade().strftime('%Y-%m-%d') for _ in range(num_rows)],
        'administration_route_code': [random.choice(['IM', 'SC', 'PO']) for _ in range(num_rows)],
        'body_site_code': [random.choice(['L', 'R', 'C']) for _ in range(num_rows)],
        'manufacturer_code': [random_string(3) for _ in range(num_rows)],
        'historical_ind': [random.choice(['00', random_string(3)]) for _ in range(num_rows)],
        'historical_lot_number': [random.choice(['00', random_alphanumeric(6)]) for _ in range(num_rows)],
    }
