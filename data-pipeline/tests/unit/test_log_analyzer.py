...
@pytest.fixture(name="sample_log_content")
def create_sample_log_content():
    ...
    # Today's ERROR
    log_entries.append(
        f"{now.strftime('%Y-%m-%d %H:%M:%S')},123 - ERROR - data_pipeline.cli - "
        f"get_school_query_information:230 - Query file not found for school Test School"
    )

    # Yesterday's ERROR
    log_entries.append(
        f"{yesterday.strftime('%Y-%m-%d %H:%M:%S')},456 - ERROR - data_pipeline.extract - "
        f"extract_function:45 - Failed to extract data"
    )

    # Last week's ERROR
    log_entries.append(
        f"{last_week.strftime('%Y-%m-%d %H:%M:%S')},789 - ERROR - data_pipeline.transform - "
        f"transform_function:78 - Invalid data format"
    )

    # Last month's ERROR
    log_entries.append(
        f"{last_month.strftime('%Y-%m-%d %H:%M:%S')},321 - ERROR - data_pipeline.load - "
        f"load_function:92 - Failed to write to output file"
    )

    # Today's INFO (should be ignored by error search)
    log_entries.append(
        f"{now.strftime('%Y-%m-%d %H:%M:%S')},654 - INFO - data_pipeline.cli - "
        f"handle_transform_command:346 - Transform command started"
    )

    return log_entries


@pytest.fixture(name="mock_log_folder")
def create_mock_log_folder(tmp_path, sample_log_content):
...
def test_get_log_files(mock_log_folder):
    ...
    analyzer = LogErrorAnalyzer(mock_log_folder)
    log_files = analyzer._get_log_files()  # pylint: disable=protected-access
...
def test_get_time_range():
    ...
    assert (
        abs((now - day_range).total_seconds() - 86400) < 10
    )  # Within 10 seconds of a day
    assert (
        abs((now - week_range).total_seconds() - 604800) < 10
    )  # Within 10 seconds of a week
    assert (
        abs((now - month_range).total_seconds() - 2592000) < 10
    )  # Within 10 seconds of 30 days
    assert all_range == datetime.min


def test_analyze_errors_last_day(mock_log_folder):
    ...
    yesterday = datetime.now() - timedelta(
        days=1, hours=1
    )  # Add buffer for test execution time
    with mock.patch.object(analyzer, "_get_time_range", return_value=yesterday):
        ...
        # (might be in different order due to timestamp sorting)
...
def test_analyze_errors_last_week(mock_log_folder):
...
def test_analyze_errors_all(mock_log_folder):
...
def test_get_error_summary(mock_log_folder):
...
def test_no_matching_errors(tmp_path):
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(
            "2020-01-01 12:00:00,123 - ERROR - module - function - Old error message\n"
        )
    ...
    with mock.patch.object(
        analyzer, "_get_time_range", return_value=datetime.now() - timedelta(hours=1)
    ):
        errors = analyzer.analyze_errors("last-day")
        assert len(errors) == 0
